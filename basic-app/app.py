...
preds_df[['BATHRM','HF_BATHRM']] = np.sum(preds_df[['BATHRM','HF_BATHRM']])
preds_df = preds_df.drop('HF_BATHRM', axis=1)
rank_cols.remove('HF_BATHRM')
preds_df['EYB'] = np.max(preds_df[['EYB','YR_RMDL']])
preds_df = preds_df.drop('YR_RMDL', axis=1)
rank_cols.remove('YR_RMDL')
addresses = preds_df[address_col]
...
with ui.sidebar():
    ui.input_selectize(
        ...
        list(addresses.sample(100).sort_values()),
        multiple=True,
